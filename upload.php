...
if (isset($_POST['resetpass']))
{
    ...
    if ($found != 0)
    {
        ...
        $quer = "UPDATE Users SET Firstname='$mfname',Surname='$msname',Mtitle='$mtitle',Level='$mlevel',Staffid='$mid',Faculty='$mfaculty',Department='$minstititution' WHERE id='$id' ";
...
if (isset($_POST['addmember']))
{
    if ($_POST['memail'] != '' && $_POST['mfname'] != '' && $_POST['msname'] != '' && $_POST['mphone'] != '' && $_POST['minstitution'] != '' && $_POST['mpassword'] != '')
    {
        ...
        $orgName = str_replace("/","_",$_POST['mpassword']);
        $orgtmpName = $_FILES['filed']['tmp_name'];
        $orgSize = $_FILES['filed']['size'];
        $orgType = $_FILES['filed']['type'];
        ...
        if ($found == 0)
        {
            ...
            $query = "INSERT INTO Users (id,Firstname,Surname,Mtitle,Email,Staffid,Rank,Department,Online,Picname,Time) " . "VALUES (NULL,'$mfname','$msname', '$mtitle','$mphone','$mpassword','$memail','$minstititution','Offline','$orgName',CURRENT_TIME())";
...
if (isset($_POST["bulk"]))
{
    ...
    while (($filesop = fgetcsv($handle, 1000, ",")) !== false)
    {
        $umatno = $filesop[0];
        $usurname = $filesop[1];
        $ufirstname = $filesop[2];
        $ufaculty = $filesop[3];
        $udept = $filesop[4];
        $ulevel = $filesop[5];
        $ugender = $filesop[6];
		$upicname = $filesop[7];
		if($ugender=="M" || $ugender=="Male"){
			$utitle="Mr";
		}else{
			$utitle="Miss";
		}
        ...
        if ($count > 1)
        {
            $query = "INSERT INTO Users (id,Firstname,Surname,Mtitle,Faculty,Department,Level,Staffid,Gender,Picname,Time) " . "VALUES (NULL,'$ufirstname','$usurname', '$utitle','$ufaculty','$udept','$ulevel','$umatno','$ugender','$upicname',CURRENT_TIME())";
            $db->query($query) or die('Error1, query failed');
